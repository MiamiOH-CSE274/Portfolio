...
Array-based list vs. Linked List
	In an array-based list, the run time of get() and set() is O(1). However, add() and remove() are O(n), since they have to cycle through all the 		elements in the array-based list. In a linked list, the run time of add() and remove is O(1) because you can use an iterator. This allows you to 		skip the iteration step, eliminating the O(n) once the iterator has been found. Get() and set() are both O(n). If you are keeping track of the size 		using an external variable, size() will always be O(1) no matter which option you chose to use. In essence, Array-based lists are good if you are 		looking for random access to the elements in the array. Linked lists do not support this. However, linked lists are much better at adding and 			removing elements.

Binary Search Tree vs. Hash Table
	When looking at a hash table, the average runtime for add(), remove(), and find() is O(1). Find() is constant because when a "key" is passed in, it 		is sent through a hash function, which changes the value by a lot. Most of the time, every item passed in will give you a separate key. Looking for 		this key is easy, since the values the hash function return are very different. If, by some weird chance, the hash table had to search through 			everything to find the correct "bucket", the run times of each of the above functions would end up being O(n). Binary Search Trees are a different 		story. Each of the functions mentioned above has an average runtime of O(log n). This is because each level on the tree has twice as many nodes as 		the level before it. If the tree is horribly unbalanced, the run times will change to be O(n). This is why keeping the tree under control is 			important.

Adjacency List vs. Adjacency Matrix
	Adjecency lists and adjacency matrix are both representations of the graph data type. An adjacency matrix is fast for looking up a specific edge in 		question (it runs in O(1)). However, the drawback is that the size is (n*n), which can be a huge number if n is a large number. An adjacency matrix 		is something that stores either a true or false boolean value for every node that is in the graph. This is what allows you to find an edge in O(1) 		time, but is also why adjacency matrixes take up so much memory. Another thing to keep in mind is that looking up all the neighbors of a certain 		node takes a long time, as it has to cycle through every node to find the ones adjacent to it (running in O(n)) time. Adjacency lists are good for 		finding the neighbors of a specific node because they store only the nodes that are connected. This makes them good at returning neighbors, but 		horrible at looking up a specific edge. Adjacency lists take up much less memory, and should be used when in a scenerio that specific edges don't 		need to be found often.
