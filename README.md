...
See my implementation of dijkstra's method here: https://github.com/MiamiOH-CSE274/Vise/blob/BlaseAndBickley/src/GameBoard.cpp (dijkstraTotal() & dijkstraRecursive())
...
Analysis 2:
For the Vise project, two possible data structures that can be used to represent the game board are an array based board and nodes with edges. By using an array-based board, you get instant access to any particular location on the board, while with the node structure, the board must be traversed to some extent in order to access the desired location on the board.

There is also a running time difference in the board constructor, particularly when it comes to assigning the pointers. With the array structure, it is constant time to assign the pointers because it is simply a mathematical calculation to assign the pointers to the space's neighbors, while the node structure would be more difficult to determine it's neighbors.
