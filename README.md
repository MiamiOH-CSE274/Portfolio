...
Adjacency lists and matrixes are used to represent the graph abstract data structure. An adjacancy matrix uses a 2D array to represent edges between rows and columns. Due to the ability to access a particular spot in the 2D array, adjacancy matrixes have O(1) add/remove edge and get weight. Adding a vertex is slower at O(n^2) because the 2D array might have to be grown in size and the old arrays values copied over to the new array. Adjacancy lists use an array of nodes, with the nodes acting as a list of connecting edges. Adding an edge, vertex, or getting the neighbors runs in O(1) time due to the ability to access any spot in the array, then simply adding the edge/vertex to the end or returning the neighbors. Removing an edge is slower due to the array not knowing if the edge exists or not, which requires iteration over the edge list, resulting in O(d) run time. Getweight is also O(d) due to the need for iteration over the adjacancy list to find the weight. Adjacancy lists are prefferable when the number of vertexes is low due to the space requirements being O(n+m) versus an adjacancy matrix which requires O(n^2) space. An adjacancy matrix is only preffered for storage of large amounts of data, such as the US highway system connections.
...
The major requirement asked for by this question is for the ability to look up students by their banner number. The choice must be a dictionary type data structure, since we must store a (key, value) pair for lookup. Another way to do it would be to use 2 data structures, but the question makes it sound like the requirements are very simple lookup, so one data structure should work well.

I believe a hash table would work best due to the ability for very fast lookup. Since students are looked up by their banner numbers, we have to make the key the banner number, and the value the students record. Student numbers are unique, so it would be very easy to use each students number to hash an index to store the students data in. This will result in O(1) lookup times, which is the most important feature asked for by the problem. The only major weakness of using a hash table to solve this problem is storage. Since the problem is talking about students, we can deduce that this data structure will be used by a school or university, and even smaller schools have a lot of students to track. At the same time, this means that storage costs should not be a problem, as an establishment such as a university will always have the funds for something as fundamental as student information storage and lookup. 

* Imagine that I'm implementing a network router. It needs to keep a queue of packets waiting to be sent out over the network, but this queue need a special ability: Different companies are going to pay me different amounts of money, and the packets from the highest paying company should be sent out first. That is, if company X paid 20 and company Y paid 10, then X's packets always get sent before Y's packets. Y only gets to send packets if X doesn't have any waiting. Which data structure (or data structures) that we learned this semester would be most appropriate? Carefully explain why.

For this problem, I believe we need an ordered set (SSet). An unordered set will cause a lot of slowdown when the highest priority companies are all sent out and we have to find the next highest. With some order, such as a BST or priority que, some operations might be slower, but for this problem, I believe an ordered or at least partially ordered data structure will always beat out an unordered one.

I believe that a max heap that acts as a priority que would be a great option for this problem. The main point to consider is how fast we can get and send out the highest priority packets. The faster we can do this, the more money we can make. In this problem, the saying time = money definitely holds true, and a heap will allow for O(1) time lookup of the highest priority packet. After all of the packets from the highest priority company are sent out, we can quickly get the next highest priority company in O(lg n) time. Adding, along with all other operation will also run in O(lg n) time, which is still very good. 
