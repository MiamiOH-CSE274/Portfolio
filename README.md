...
* **If I needed a data structure to store student records so that I could look students up by Banner number, which data structure (or data structures) that we learned this semester would be most appropriate? Carefully explain why.**
  ...
  I believe the best data structure for this problem would be a hashtable. Since not much information was given for the problem, I made a series of assumptions. The first is that looking students up is the most important operation that needs to be performed. This means that, optimally, this will be a very fast method. With hashtables, a lookup can be performed in O(1) time. This would be done by hashing the student's banner number to find the index in the array. Secondly, I assume that adding and removing students is the next biggest operation needed to be performed. Both the add and remove operation has an average running time of O(1) assuming we'll be smart and develop a well structured hash function and are able to predict the number of students we'll be adding. If both of these assumptions are true, we could develop a hashtable with a proper load factor and reduce the amount of collisions. If we did this poorly, the worst case running time of all of our methods could be O(n) if we used linear probing as our collision resolution strategy! The last assumption is that we know the banner id of a student. That is, when we want to find the student, we already know their banner id. I feel like this is often the case as students are required to input their banner id when making changes to their information, or our professor/advisor will have access to an info page about us containing our banner number.

  Inside of the hashtable we would store key value pairs as is normal. Our key would be the student's banner number, but for the value we would have to do something a little special. It's likely the case with our data that each banner id would have more than just one value it needs to keep track of. For example, for each banner id you'd probably want to store the student's name, their GPA, and maybe their major. To do this, we would need to develop some kind of container object like a record. This record would hold the information about the student. If could also includes things like arrays for our classes and things like that. Since the requirement for this answer is that we should be able to look students up by their banner id, I will not get into possible running times for getting the information out of the record.  Since we're able to access the student's records by hashing their banner id it'd allow us to retrieve information on a particular student in constant O(1) time. 
